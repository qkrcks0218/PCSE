...
ModelAgL <- ModelDgAL <- ModelYgA1DL <- list()
for(ss in 1:CF){
  # ModelAgL[[ss]] <- 
  ...
  ModelYgA1DL[[ss]] <-
    MySL(Data[ intersect(SS.Index[[ss]],which(A==1&D==0)) , ],
         locY=Y.pos,
         locX=c(W.pos,Z.pos,X.pos),
         Ydist=gaussian(),
         SL.list=SL.hpara$SLL,
...
for(ss in 1:CF){
  ...
  Data.Temp <- Data[ SS.Index[[ss]] , c(W.pos,Z.pos,X.pos)]
  
  YgA1D0L.NoCF[ SS.Index[[ss]] ] <- 
    predict(ModelYgA1DL[[ss]], newdata=Data.Temp)$pred
...
q11.NoU <- (1/PrA0)*(D0gA0L.CF/D0gA1L.CF)
h11.NoU <- YgA1D0L.CF
...
write.csv(Result1,
          sprintf("%sResult_NoU_exp_N%s_B%0.5d.csv",
                  FOLDER,
                  N, 
                  seed.data),
          row.names=F)
